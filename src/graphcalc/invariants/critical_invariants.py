...
def vertex_critical_set(G, f, *, kind="change"):
    ...
    >>> vertex_critical_set(G, gc.independence_number, kind="change")  # doctest: +ELLIPSIS
    {...}
    """
...
def vertex_critical_number(G, f, *, kind="change"):
    ...
    >>> import graphcalc as gc
    >>> G = nx.path_graph(6)
    >>> vertex_critical_number(G, gc.independence_number, kind="change")
    0
    """
...
def vertex_deletion_max_jump(G, f):
    ...
    >>> import graphcalc as gc
    >>> G = nx.path_graph(6)
    >>> vertex_deletion_max_jump(G, gc.zero_forcing_number)
    1
    """
    deltas = vertex_deletion_deltas(G, f)
...
def edge_deletion_deltas(G, f):
    ...
    >>> import graphcalc as gc
    >>> G = nx.cycle_graph(5)
    >>> d = edge_deletion_deltas(G, gc.chromatic_number)
